...
export function CameraPicker({
  ...
  // Enumerate devices when popover opens
  useEffect(() => {
  ...
  return (
    <div ref={popoverRef} className="fixed bottom-4 left-4 z-50">
      ...
      {/* Popover */}
      {isOpen && (
        <div className="absolute bottom-14 left-0 w-64 animate-in fade-in slide-in-from-bottom-2 duration-200">
          <div className="rounded-lg border border-white/10 bg-black/90 p-2 shadow-2xl backdrop-blur-xl">
            ...
            {/* Mobile hint */}
            {devices.length > 1 && (
              <div className="mt-2 border-t border-white/10 px-2 pt-2 text-xs text-white/40">
                Tap to switch between front and back cameras
              </div>
            )}
          </div>
        </div>
